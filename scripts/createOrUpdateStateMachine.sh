...
PUBLISH_OPTIMAL_LINEUP_LAMBDA_ARN=$(echo "$LAMBDA_FUNCTIONS" | jq -r '.Functions[] | select(.FunctionName | contains("PublishOptimalLineupFunction"))' | jq '.FunctionArn' | tr -d '"')
...
echo "PUBLISH_OPTIMAL_LINEUP_LAMBDA_ARN=$PUBLISH_OPTIMAL_LINEUP_LAMBDA_ARN"
echo "STATE_MACHINE_DEFINITION_FILE=$STATE_MACHINE_DEFINITION_FILE"
...
sed -i "s/REPLACE_ME_WITH_PUBLISH_OPTIMAL_LINEUP_LAMBDA_ARN/$PUBLISH_OPTIMAL_LINEUP_LAMBDA_ARN/g" $STATE_MACHINE_DEFINITION_FILE
